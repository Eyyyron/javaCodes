...
public class ProductRecordingUsingStream {
        public static void main(String[] args) {
            ...
            try{
                program = new FirstYearShortTermAndSecondYearFirstTerm.finals.ProductRecordingUsingStream();
                program.run();
            } catch (Exception x){
                x.printStackTrace();
            }
        ...
        public void run() throws Exception{
             ...
             */
            System.out.println("\n3. Products with unit price that is less than 170");
            productList.stream().filter(p -> p.getUnitPrice()<170).forEach(p ->
                    System.out.println(p));

            System.out.println("\n4. Products with unit price that is greater than 170");
            productList.stream().filter(p->
                    p.getUnitPrice()>170).forEach(System.out::println);
            ...
            (Note that the return type of terminal operation count() is long.
            Hence, the type casting is needed if the result is assigned to an int variable)
             */
            int counter = (int) productList.stream().filter(p->p.getUnitPrice()
            <170).count();
