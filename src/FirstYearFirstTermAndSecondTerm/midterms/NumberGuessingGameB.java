/**
 ...
 Problem: Modify the given program called NumberGuessingGameA.java such that the player
 will be allowed to guess the secret number for 5 chances. NumberGuessingA.java is included
 in the additional demo programs on creating methods. ( Note that the given program allows
 the user to guess the secret number for 3 chances).

 This program allows the user to guess a randomly generated integer between 1 and 100 (inclusive).
 The user is given 5 chances to guess the number. For every guess that is not correct, the program
 tells the user to try something higher or lower, whichever is appropriate.
...
public class NumberGuessingGameB {
    ...
    //Returns an integer in the range 1 to upperLimit that is entered by the program user
